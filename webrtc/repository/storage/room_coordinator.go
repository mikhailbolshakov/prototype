...
type roomCoordImpl struct {
	c *container
}

func (r *roomCoordImpl) GetOrCreate(ctx context.Context, meta *domain.RoomMeta) (bool, error) {

	roomId := meta.Id
	found := false

	l := log.L().Cmp("webrtc").Mth("coordination").C(ctx).F(log.FF{"room": roomId})
...
func (r *roomCoordImpl) Close(ctx context.Context, roomId string) {

	l := log.L().Cmp("webrtc").Mth("coordination.close-room").F(log.FF{"room": roomId})
